...
AdamW::AdamW() : adam_props(PropsB1(), PropsB2(), PropsEpsilon(), TorchRef(), PropsWeightDecayW()) {
  /** default properties */
...
/**
 * @brief bias-corrected learning rate (Adam)
 */
static double getUpdatedLearningRate(
  unsigned int iteration, double ll,
  const std::tuple<PropsB1, PropsB2, PropsEpsilon, TorchRef, PropsWeightDecayW>
    &adam_props) {
...
void AdamW::applyGradient(RunOptimizerContext &context) {
  ...
  // Decoupled weight decay (L2): w = w - lr * wd * w
  if (weight_decay > 0.0) {
  ...
  double lr_t = getUpdatedLearningRate(context.getIteration(),
                                       context.getLearningRate(), adam_props);
