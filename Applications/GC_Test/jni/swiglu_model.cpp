...
std::string weight_file = "swiglu_weights.bin";
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--seq_len" && i + 1 < argc) {
            ...
            std::cout << "Usage: " << argv[0] << " [options]\n"
                      ...
                      << "  --model_dim <int>      Model dimension (default: 512)\n"
                      << "  --num_heads <int>      Number of attention heads (default: 8)\n"
                      << "  --num_layers <int>     Number of transformer layers (default: 6)\n"
                      << "  --ffn_dim <int>        FFN dimension (default: 2048)\n"
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        // Save weights if specified (legacy support)
        ...
        // Save weights for normal mode
        if (!enable_checkpoint) {
            std::cout << "[INFO] Saving weights to " << weight_file << std::endl;
            try {
                model->save(weight_file, ml::train::ModelFormat::MODEL_FORMAT_BIN);
            } catch (const std::exception &e) {
                std::cerr << "[ERROR] Failed to save weights: " << e.what() << std::endl;
            }
        }
        ...
        // SwiGLU FFN: gate + up + down (3 projections instead of 2)
