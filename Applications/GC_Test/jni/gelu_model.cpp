...
std::string weight_file = "gelu_weights.bin";
...
public:
    ...
    static void printMemoryInfo(const std::string& stage) {
        std::ifstream status_file("/proc/self/status");
        std::string line;
        size_t vm_peak = 0, vm_size = 0, vm_rss = 0;
        ...
        std::cout << "[MEMORY] " << stage << " - Peak: " << (vm_peak / 1024) 
                  << " MB, Current: " << (vm_size / 1024) << " MB, RSS: " 
                  << (vm_rss / 1024) << " MB" << std::endl;
    }
    ...
    // Iteration stats are now printed from neuralnet.cpp
};
...
std::unique_ptr<ml::train::Model> create_model() {
    ...
    for (int i = 0; i < num_layers; i++) {
        ...
        try {
            ...
            // FFN: FC -> GELU -> FC
            std::cout << "[DEBUG] Adding FFN fc1 for layer " << i << std::endl;
            model->addLayer(
                ml::train::createLayer("fully_connected", {
            ...
            std::cout << "[DEBUG] Adding FFN fc2 for layer " << i << std::endl;
            model->addLayer(
                ml::train::createLayer("fully_connected", {
            ...
            std::cout << "[DEBUG] Adding add2 (residual after FFN) for layer " << i << std::endl;
            model->addLayer(
                ml::train::createLayer("addition", {
                    "name=layer" + std::to_string(i) + "/add2",
                    "input_layers=layer" + std::to_string(i) + "/ln_multiout2(1), layer" + std::to_string(i) + "/fc2"
                })
            );
    ...
    try {
        ...
        // TEMPORARY: Skip pooling2d to test if it causes segfault
        std::cout << "[DEBUG] Skipping pooling2d layer for debugging..." << std::endl;
    ...
    if (enable_checkpoint) {
        ...
        for (int i = 0; i < num_layers; i++) {
            std::vector<std::string> block_layers = {
                ...
                "layer" + std::to_string(i) + "/fc1",
                "layer" + std::to_string(i) + "/fc2",
...
public:
    SimpleDataGenerator(int bs, int sl, int num_batches) 
        ...
        for (int i = 0; i < data_size_; i++) {
            input_data_[i] = static_cast<float>(rand()) / RAND_MAX;
            label_data_[i] = static_cast<float>(rand()) / RAND_MAX;
        }
    }
...
SimpleDataGenerator* getDataGenerator() {
    return new SimpleDataGenerator(batch_size, seq_len, number_of_db);
}

// Dataset callback
int dataset_cb(float **input, float **label, bool *last, void *user_data) {
    auto data = reinterpret_cast<SimpleDataGenerator*>(user_data);
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        // save_path removed to avoid saving model every epoch (which takes extra time)
        ...
        auto data_generator = getDataGenerator();
        auto train_dataset = ml::train::createDataset(
        ml::train::DatasetType::GENERATOR, dataset_cb, data_generator);
        ...
        // Training with detailed iteration tracking
        timer_start = PerformanceMonitor::startTimer();
        ...
        size_t embedding_params = seq_len * model_dim; // FC embedding
        size_t attention_params = num_layers * (4 * model_dim * model_dim); // Q,K,V,O projections
        size_t ffn_params = num_layers * (model_dim * ffn_dim + ffn_dim * model_dim);
        size_t total_params = embedding_params + attention_params + ffn_params;
        
        std::cout << "\nApproximate Model Parameters (GELU FFN):" << std::endl;
        std::cout << "  - Embedding: " << embedding_params << std::endl;
        std::cout << "  - Attention: " << attention_params << std::endl;
        std::cout << "  - GELU FFN: " << ffn_params << " (2 projections: fc1+fc2)" << std::endl;
